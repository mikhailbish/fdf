...
/*   Updated: 2025/01/18 22:39:56 by mbutuzov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_data(t_map dim, mlx_image_t *image)
{
	...
	convert_3dto2d(&dim);
	get_max_min_x_display(&dim);
	get_max_min_y_display(&dim);
	offset_x = WIDTH / 2 - dim.max_x / 2;
	offset_y = HEIGHT / 2 - dim.max_y / 2;
	shift_x_2d(dim, dim.coords_display, offset_x);
	shift_y_2d(dim, dim.coords_display, offset_y);
	put_lines(image, dim);
}
